...
export default function Cart() {
  ...
  let { getCart, cart, updateProductCount, loading, deleteProduct, clearCart } = useContext(CartContext)
  ...
  return <>
    {loading ? <Loading /> : <div>
      {cart ? (
              ...
              <tr>
                <td colSpan={5} className="text-center py-4">
                  <div className="flex justify-center gap-4">
                    <Link to={'/checkout'} className='btn btn-green hover:bg-main hover:text-white flex items-center justify-center' style={{ fontSize: '1.2rem' }}>
                      Checkout
                      <i className="fa-regular fa-credit-card fa-xs ms-2"></i>
                    </Link>
                    <button onClick={clearCart} className='btn btn-red hover:bg-red-700 text-white flex items-center justify-center' style={{ fontSize: '1.2rem' }}>
                      Clear Cart
                      <i className="fa-solid fa-trash fa-xs ms-2"></i>
                    </button>
                  </div>
                </td>
              </tr>
        ...
        <h2 className='py-5 text-2xl text-green-600 font-semibold text-center'>Cart Is Empty</h2>
      )}
    </div>}
  </>
}
